...
## Credit Beta-Market Polish - 2026-05-28

- Current-task browser constraint honored: reused existing local browser page `fc312d68-4538-4147-8151-d089161db755` at `http://127.0.0.1:5175/credit`; no new browser, headless browser, or separate Playwright browser was opened.
- Local baseline before beta seed: wallet `0xf76e...71a3`, Arbitrum Sepolia, `0.1170 ETH`; canonical market public mirrors showed supplied `$0.009`, borrowed `$0.0025`, borrowable/beta liquidity `$0.0065`, utilization `27.8%`; reputation endpoint showed active tier with score `10` from Pay + Credit signals.
- Implemented lightweight beta borrow caps in `frontend/obscura-os-main/src/hooks/useBetaBorrowLimit.ts`: beta target `20 ocUSDC`, pool label `Obscura Treasury Pool`, caps `new=0.25`, `active=2`, `steady=5`, `reliable=8` ocUSDC, plus a 25% live-liquidity guard once pool liquidity exceeds 1 ocUSDC. This is a product/UI beta guard only; contract LLTV and liquidity checks remain the hard on-chain enforcement.
- Wired beta caps into Borrow and Setup flows. Borrow now shows protocol max separately from `Beta borrow limit`, uses the lower of protocol max, live liquidity, and remaining reputation cap, disables over-cap submissions before wallet signature, and explains that Pay/Credit/Vote reputation can raise limits as the pool grows. SetupSheet uses the same cap model and blocks over-cap guided borrow requests.
- Polished canonical beta-market UX without adding fake markets or fake TVL: Overview/Borrow/Earn now lead with `Beta liquidity`, `Early Access Liquidity`, `Obscura Treasury Pool`, `Beta target 20 ocUSDC`, `Canonical beta market`, and the narrative `Private money -> private reputation -> private credit -> private governance`. Earn direct supply is labeled `Supply to Beta Liquidity Pool` / `Supply to Beta Pool`; legacy/vault paths remain demoted.
- Verified local UI after edits: Overview displayed the new treasury pool card and target, Borrow displayed `Beta borrow limit 0.004 ocUSDC` while the pool was still tiny and liquidity-capped, and Earn displayed the beta pool supply path. VS Code diagnostics were clean for all edited frontend files.
- First attempt to seed `20 ocUSDC` through Earn failed before any wallet signature or transaction hash. Error surfaced from CoFHE SDK key storage: `Failed to rehydrate keys store | Caused by: onRehydrateStorage: Error rehydrating keys store: Error: Iframe storage hub did not initialize within the allowed time before calling method "indexDBKeyval.get". Waited 30000ms.` No liquidity moved and the market mirrors remained unchanged.
- Added a minimal retry/user-friendly error around FHE encryption storage-hub startup failures in `src/lib/fhe.ts`. Retried the same `20 ocUSDC` supply in the same local tab; the retry still failed before wallet signature with `CoFHE local key storage is still starting. Refresh Credit and retry the private action.` No tx hash and no liquidity moved.
- Root cause investigation found the installed CoFHE web SDK default key storage uses external iframe-shared-storage at `https://iframe-shared-storage.vercel.app/hub.html` with a 30s ready timeout. Local browser IndexedDB was available, but no CoFHE key DB was created; the iframe hub failed to initialize in this local profile before private writes.
- Patched `src/lib/fhe.ts` to pass a same-origin localStorage-backed `fheKeyStorage` into `createCofheConfig`, avoiding the external iframe storage hub for public FHE key/CRS cache. Diagnostics are clean. Next step is to reload the same local Credit tab and retry the real `20 ocUSDC` beta-pool supply.
- Same local-tab retry after same-origin CoFHE key storage succeeded. Earn supplied `20 ocUSDC` into the canonical market. UI showed `Supplied 20 ocUSDC to market.` and pool supplied updated from `$0.009` to `$20.01`; no fake TVL was introduced.
- Beta seed txs recovered from recent Arbitrum Sepolia logs:
	- Token confidential transfer user -> canonical market: `0x0cdc9dd303cfbc956a8f323c37f8d54437746d03760c13a26dc6898bba285fda` (block `271760428`).
	- Market `Supplied(user)` accounting: `0xe78c21a231037a981d4de50b78c87feeb4b3158431b56dd438382b39a101ebc6` (block `271760531`).
- Post-seed canonical market mirrors: `totalSupplyAssets=20009000` (`20.009 ocUSDC`), `totalBorrowAssets=2500` (`0.0025 ocUSDC`), `utilizationBps=1`, `borrowersLength=1`. UI showed beta liquidity `$20.01` and utilization `0.0%`.
- Borrow-cap verification: Borrow tab displayed the seeded pool (`Beta pool supplied $20.01`, `Beta liquidity $20.01`) and reputation-aware beta limit. Over-limit input `2.1 ocUSDC` displayed pre-signature warnings for both protocol max and beta cap. A safe in-bounds borrow of `0.001 ocUSDC` succeeded from the beta pool; tx `0x24b92686f8d4ad88f8e46a810d64ce52d578b7704172e199f60ec3f81adbf371` (block `271761483`).
- Repay verification: repaid the same `0.001 ocUSDC` test debt successfully; tx `0x1b6126a63f6f8daec0edf5d9087c99603f319fe8aabc7b9ea775e65cdad0ca3f` (block `271761767`). Debt returned to `0.0025 ocUSDC`.
- Collateral verification: supplied `0.001 ocUSDC` additional collateral via direct two-step flow. Token confidential transfer tx `0xc643f2c650f36cbc0b3af669db277f74d552d4e1c8be1da56126eaea97bb42fb` (block `271761936`), market `CollateralSupplied(user)` tx `0xf320505fcc0c10349d48e16d6ee6fe7b8300db1463140fa4bc98d02689aca5df` (block `271762043`). Final mirrors after flow tests: `totalSupplyAssets=20009000`, `totalBorrowAssets=2500`, `utilizationBps=1`, `getPlainCollateral=9000`, `getPlainBorrow=2500`, `maxBorrowable=5240`.
- Reveal/privacy verification: Position stayed masked by default after page navigation and txs. Explicit `Reveal all` showed `20.009 ocUSDC` supplied, `0.0025 ocUSDC` borrowed, and `0.009 ocUSDC` collateral; `Hide all` returned all three tiles to masked state. No auto-decrypt on mount was introduced.
- Realtime/activity/reputation verification: Overview and Risk feeds showed new indexed Credit rows for `Credit supplied`, `Credit borrow opened`, `Credit repayment recorded`, and `Credit collateral supplied` with realtime on. Reputation UI refreshed to tier `Steady`, score `14`, with Pay reliability `5`, Credit discipline `7`, Governance `0`. A manual direct browser fetch to the Render reputation URL from localhost hit CORS, but the in-app reputation panel itself loaded and refreshed successfully.
- Notification verification: Risk tab showed push enabled, browser permission granted, service worker ready, and Credit event types enabled. Test notification returned `Browser displayed. Server sent 1/1.` Messages stayed generic and amount-free.
- Mobile/onboarding verification: 390px mobile viewport showed no horizontal overflow during Risk check. Setup onboarding copy now says `Private money from Pay builds private reputation, then unlocks private Credit inside one ocUSDC system`, `Pay-backed ocUSDC · beta liquidity pool · no faucet`, and the borrow step shows the beta borrow limit and `Obscura Treasury Pool` tier explanation. Added a tiny scroll/stacking patch around the top setup CTA in `CreditPage.tsx`; Playwright's strict trial click still reported an interception from the surrounding layout, but forced/coordinate dispatch opened the sheet and the visible onboarding content is correct. Track this as a minor Playwright/mobile-click quirk for further polish if it reproduces in hand testing.
- Validation after beta polish: VS Code diagnostics are clean for edited frontend/test/docs files. `npm run test -- src/test/pay-final-p0.test.ts` passed with 19 tests after updating the stale `Advanced/Testnet` assertion to canonical beta-market copy. `npm run build` in `frontend/obscura-os-main` passed with existing Browserslist/Rollup chunk warnings only.
- Documentation update completed in `credit_wave5_protocol_bible_v1.md`: added sections 54-58 covering beta liquidity architecture, treasury pool seed/model, privacy-preserving liquidity design, beta borrow-cap model, reputation integration, canonical-first market philosophy, user testing flow, validation txs, limitations, and build/test evidence.
...
# Credit Protocol Bible Expansion - 2026-05-28

## Live deployed audit inputs

- Followed the user's constraint before editing: read only `memory_credit_5.md` and `credit_wave5_protocol_bible_v1.md`, then inspected the already-open live browser session at `https://obscura-os-nine.vercel.app/credit`.
- Desktop live audit covered Overview, Borrow, Position, Earn, Liquidations, Risk, SetupSheet, Settings, reveal entry points, activity/reputation, notifications, and connected/disconnected wallet states. Connected state used wallet `0xf76e...71a3` on Arbitrum Sepolia.
- Live canonical metrics observed: `Supplied $0.009`, `Borrowed $0.0025`, `Borrowable liquidity $0.0065`, `Utilization 27.8%`, `LLTV 86%`, `Max borrowable 0.0044 ocUSDC`, `Health factor 2.75`.
- Live shared systems observed: Credit activity rows rendered in the feed (`Credit borrow opened`, `Credit collateral supplied`, vault deposit/withdraw, supply withdrawn), reputation active with total score 10 (`Pay reliability 5`, `Credit discipline 3`), push enabled, browser permission granted, service worker ready, Credit event types enabled.
- Mobile live audit at 390x844 verified no horizontal overflow (`scrollWidth == clientWidth`) and visible bottom nav labels `Home`, `Borrow`, `Pos`, `Earn`, `Liq`, `Risk`. The captured mobile Settings sheet showed Advanced markets, Shared notifications, and Credit alert categories in a full-height responsive drawer.

## Documentation additions made

- Expanded `credit_wave5_protocol_bible_v1.md` only; no new documentation file was created.
- Added §41 `Live Deployed UI Verification - Vercel Audit Addendum` with runtime state, per-tab live behavior, Setup/Settings observations, and screenshot/UI reference anchors.
- Added §42 `Frontend Provider and State Architecture` with app provider graph, public/private state split, wallet/chain invariants, and reveal-state invariants.
- Added §43 `UI -> Hook -> Contract Flow Matrix` mapping every major Credit UI workflow to components, hooks, contracts/APIs, events/data effects, and privacy behavior.
- Added §44 `Complete User Workflow Maps` with sequence/flow diagrams for canonical setup, reveal, activity/reputation, and notifications.
- Added §45 `Backend Indexer, Retry, Realtime, and Recovery Deep Dive` documenting watcher topology, retry model, worker RPC fallback recovery, duplicate handling, backfill, and reorg limitations.
- Added §46 `Supabase Lifecycle and Schema Semantics` covering activity rows, reputation rows, push subscription/prefs rows, and realtime UX contracts.
- Added §47 `Runtime and Infrastructure Architecture` showing Vercel, wallet provider, Render API/worker, Supabase, RPC mesh, and Arbitrum Sepolia contract runtime.
- Added §48 `Expanded Threat Model and Security Assumptions` for wrong-chain state, stale wallet clients, reverted receipts, RPC outages, CORS drift, feed inference, notifications, mobile action safety, and legacy/testnet confusion.
- Added §49 `Failure Edge Cases and Recovery Runbooks` for rate limits, CoFHE settle timeouts, router revert, decrypt context errors, missing activity, stale reputation, push failures, mobile overflow, wrong network, and legacy market selection.
- Added §50 `Governance Integration Flows` for parameter approvals, new market candidates, and emergency governance actions.
- Added §51 `Production Readiness and Mainnet Migration Addendum` with current readiness evidence and mainnet deltas.
- Added §52 `Complete Source and Code Reference Map` linking runtime questions to the canonical source files.
- Added §53 `Additional Architecture Decisions` covering direct-market setup, Pay-backed ocUSDC reveal, shared data plane, mobile Settings role, worker RPC fallback, and reverted receipt handling.

## Architectural gaps now covered

- Live UI evidence is now documented alongside protocol architecture, including exact deployed tab behavior and mobile state.
- The frontend provider/state graph is explicit; public reads, encrypted reveals, Supabase realtime, notification prefs, and wallet/chain state are separated.
- Every primary product workflow now has a UI-to-hook-to-contract/API mapping and privacy expectation.
- Shared activity/reputation/notification architecture is documented as an end-to-end lifecycle, not just a backend feature list.
- Worker failure/recovery behavior is now protocol-grade: RPC fallback, retry/backfill, duplicate handling, health signals, and reorg limitations are recorded.
- Mainnet readiness gaps are clearer: finality-aware indexing, production RLS, paid RPC monitoring, legacy/testnet removal, hook/router separation, and liquidation gating are explicit gates.
- The canonical/legacy boundary is clearer: direct Pay-backed market paths are primary; router/faucet/hook testnet controls remain Advanced/Settings surfaces only.
