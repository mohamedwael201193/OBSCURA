...
22. **Stealth pay `announce` rate-limit fix (round 2 — sign-once/retry-broadcast).** Root cause revisited: round 1 removed the simulation round-trip but `writeContractAsync` itself calls `eth_sendRawTransaction` via the RPC. If the RPC returns HTTP 429 at _send_ time, MetaMask marks the transaction Failed. There is no automatic retry without another MetaMask popup. Fix: replaced `writeContractAsync` for the `announce` step with a **sign-once / retry-broadcast** pattern — `encodeFunctionData` builds the calldata, `walletClient.signTransaction` produces signed bytes (one MetaMask popup), then `publicClient.sendRawTransaction` is retried inside `withRateLimitRetry(retries=5, baseDelay=3s)` until the RPC accepts it. No extra user prompts on retry.
