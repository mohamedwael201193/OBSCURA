      ...
      <div className="pt-24 px-6 pb-16 max-w-[1400px] mx-auto">
        ...
        >
          ...
          <p className="text-sm font-body text-muted-foreground mt-2 max-w-lg">
            Coercion-resistant encrypted governance. Votes are FHE-encrypted — no one sees individual choices. Only the aggregate tally is revealed after the deadline.
          </p>

          {/* Benefits section */}
          <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { label: "Privacy", desc: "Individual votes stay encrypted" },
              { label: "Coercion-Resistant", desc: "Revote anytime before deadline" },
                ...
                {privacyOpen && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                    <div className="p-4 space-y-3">
                      {[
                        {
                          handle: OBSCURA_VOTE_ADDRESS
                            ? `${OBSCURA_VOTE_ADDRESS.slice(0, 6)}...vote`
                            : "Not deployed",
                          type: "euint64",
                          label: "Individual Vote (option index)",
                          acl: ["Contract + Voter"],
                        },
                        {
                          handle: OBSCURA_VOTE_ADDRESS
                            ? `${OBSCURA_VOTE_ADDRESS.slice(0, 6)}...tally`
                            : "Not deployed",
                          type: "euint64 × N",
                          label: "Per-option Tally (aggregate)",
