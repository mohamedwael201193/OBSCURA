...
| 56 | **Escrow fund() ciphertext target fix** — `fund()` reverted because `FHE.asEuint64()` calls `Impl.verifyInput(input, msg.sender)` where `msg.sender = escrow contract address`. The ciphertext signature from `encryptAmount()` was being signed for the **user wallet** address, not the escrow contract. Fix: `encryptAmount()` now accepts an optional `forContract` parameter which calls `builder.setAccount(contractAddress)` so the CoFHE zkVerifier signs the ciphertext for the escrow contract. Both auto-fund and manual `fund()` now pass `REINEIRA_ESCROW_ADDRESS` as the target. `encryptAddressAndAmount` for `create()` continues to use the wallet account (escrow contract calls `FHEMeta.asEuint64(input, msg.sender)` where `msg.sender = EOA`). | `src/lib/fhe.ts`, `src/hooks/useCUSDCEscrow.ts` | ✅ Done |
