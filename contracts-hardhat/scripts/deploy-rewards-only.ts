...
async function main() {
  ...
  console.log(`Deploying ObscuraRewards to ${network.name}...`);
  console.log(`Deployer: ${deployer.address}`);
  console.log(`voteContract: ${newVoteAddress}`);

  const ObscuraRewards = await ethers.getContractFactory("ObscuraRewards");
  const rewards = await ObscuraRewards.deploy(newVoteAddress);
  await rewards.waitForDeployment();
  const rewardsAddress = await rewards.getAddress();
  console.log(`\nObscuraRewards deployed to: ${rewardsAddress}`);
  ...
  data.ObscuraRewards = rewardsAddress;
  ...
  if (fs.existsSync(envFile)) {
    ...
    env = env.replace(/VITE_OBSCURA_REWARDS_ADDRESS=.*/, `VITE_OBSCURA_REWARDS_ADDRESS=${rewardsAddress}`);
