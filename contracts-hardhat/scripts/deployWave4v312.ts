// Wave 4 v3.12 — Fix: Add FHE.allowThis(handle) before every confidentialTransfer
//
// Root cause: cUSDC's confidentialTransfer(address, uint256) calls
// isAllowed(handle, msg.sender) as its FIRST operation. msg.sender is the
// market/vault calling it. Without FHE.allowThis(handle), the market/vault
// has no persistent ACL entry → isAllowed returns false → revert 0x0d5fdb3f.
//
// Affected functions fixed:
//   ObscuraCreditMarket: borrow(), withdraw(), withdrawToVault(), withdrawCollateral()
//   ObscuraCreditVault:  withdraw(), reallocateSupply()
//
// Run:
//   npx hardhat run scripts/deployWave4v312.ts --network arb-sepolia
...
async function main() {
  ...
  // ─── Wire markets: auction + repay routers ─────────────────────────────────
  ...
  // Conservative vault: 77% cUSDC, cOBS, cWETH (lower risk)
  ...
  // Aggressive vault: 86% cUSDC, cOBS, cWETH (higher yield)
  ...
  dep["wave4v312DeployedAt"]             = new Date().toISOString();
  fs.writeFileSync(deployPath, JSON.stringify(dep, null, 2));
  console.log("\ndeployments/arb-sepolia.json updated ✓");

  // ─── Summary ──────────────────────────────────────────────────────────────
  console.log("\n═══════════════════════════════════════════════════════════════");
  console.log("Wave 4 v3.12 — FHE.allowThis fix for confidentialTransfer ACL");
  console.log("═══════════════════════════════════════════════════════════════");
  ...
  console.log(`VITE_OBSCURA_CREDIT_MARKET_COBS_ADDRESS=${mOBSAddr}`);
  console.log(`VITE_OBSCURA_CREDIT_MARKET_CWETH_ADDRESS=${mWETHAddr}`);
