/**
 * index.ts — Obscura Pay Event Indexer
 *
 * Watches on-chain events from all active Obscura Pay contracts on Arbitrum Sepolia
 * and persists them to Supabase Postgres for the activity feed.
 *
 * Architecture:
 *   viem watchEvent → insertActivity(db) → Supabase Realtime → frontend useActivityFeed
 *
 * Backfill:
 *   On startup, catches up from last indexed block per contract.
 */
...
import { insertActivity, getLastIndexedBlock } from "./db.js";

dotenv.config();

// ─── Contracts ────────────────────────────────────────────────────────────────
const CONTRACTS = {
  // Active contracts (use for new events)
  ...
  // Legacy contracts (indexed for historical activity only)
...
async function backfill(
  ...
  if (fromBlock === 0n) return; // No prior indexing — let watchEvent handle from head
...
async function main(): Promise<void> {
  console.log(`[indexer] Starting Obscura Pay Indexer`);
  console.log(`[indexer] RPC: ${RPC_URL}`);
  console.log(`[indexer] Chain: Arbitrum Sepolia (${CHAIN_ID})`);

  // Backfill gaps for each contract (active + legacy)
  ...
  // Start live watchers (active contracts only)
  const unwatchers = [
  ...
  // Graceful shutdown
  process.on("SIGINT",  () => { unwatchers.forEach((u) => u()); process.exit(0); });
  process.on("SIGTERM", () => { unwatchers.forEach((u) => u()); process.exit(0); });
}
