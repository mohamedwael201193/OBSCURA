 ...
 *
 * Contracts indexed (active, Arbitrum Sepolia):
 *   ObscuraPay                  → PaymentSent, PaymentReceived
 *   ObscuraPayStreamV3 (active) → StreamCreated, StreamCancelled, CycleSettled
 *   ObscuraPayStreamV2 (legacy) → StreamCreated, StreamCancelled, CycleSettled
 *   ObscuraInvoice              → InvoiceCreated, InvoicePaid
 *   ObscuraConfidentialEscrow   → EscrowDeposited, EscrowReleased, EscrowRefunded
 *   ObscuraInsuranceSubscription → Subscribed, Cancelled, Consumed
 *   ObscuraStealthRegistry      → StealthAddressRegistered
 */
...
// Used for both V2 and V3 (same event signatures)
